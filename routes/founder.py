...
from schemas.offer import OfferResponse, OfferActionCreate
...
@router.get("/{founder_id}/pitches/{pitch_id}/offers", response_model=List[OfferResponse])
async def get_founder_pitch_offers(
    founder_id: int,
    pitch_id: int,
    db: AsyncSession = Depends(get_db)
):
    """Get all offers for a founder's pitch"""
    # Verify founder has access to this pitch
    access_check = await db.execute(
        text("""
            SELECT 1 FROM founder_pitch_relationship
...
async def take_offer_action(
    ...
    """Take action on an offer (accept/reject)"""
    # Verify offer exists and founder has access
    offer_check = await db.execute(
        text("""
            SELECT o.status, o.pitch_id
            FROM offers o
            JOIN founder_pitch_relationship fpr ON o.pitch_id = fpr.pitch_id
            WHERE o.id = :offer_id
            AND fpr.founder_id = :founder_id
        """),
        {
            "offer_id": offer_id,
            "founder_id": founder_id
    ...
    if action.action == 'withdraw':
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Founders cannot withdraw offers"
        )
    ...
    await db.execute(
        text("""
            ...
            VALUES (
                :offer_id, :action, :founder_id, :notes, CURRENT_TIMESTAMP
            )
        """),
        {
            "offer_id": offer_id,
    ...
    await db.commit()
    return {"status": "success", "message": f"Offer {action.action}ed successfully"}

@router.get("/{founder_id}/pitches/{pitch_id}/bills", response_model=List[BillResponse])
async def get_founder_pitch_bills(
    founder_id: int,
    pitch_id: int,
    db: AsyncSession = Depends(get_db)
):
    """Get all bills for a founder's pitch"""
    # Verify founder has access to this pitch
    access_check = await db.execute(
        text("""
            SELECT 1 FROM founder_pitch_relationship
